...
def train(context: MLClientCtx, training_files: mlrun.DataItem):
    ...
    with open(model_metadata_path, 'r') as model_md_content:
        ...
        learning_rate = models_metadata[0]["training"]["parameters"]["learning_rate"]
        epochs = models_metadata[0]["training"]["parameters"]["epochs"]
    ...
    for type in types:
        ...
        for b in bys:
            ...
            print( f"{type}_{b}_filtered.test.txt")
            print(test_df)
            with open(filtered_results, "w") as f:
                for line in test_df.fillna("")["texts"]:
                    ...
                    print(line)
                    predict_tuple = model.predict(str(line))
                    label = predict_tuple[0][0]
                    prob = predict_tuple[1][0]
                    f.write(label + "\t" + str(prob) + "\n")
